...
/**
 * Servlet implementation class QA_register
 */
@WebServlet("/QA_register")
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 1, // 1 MB
maxFileSize = 10737418240L,     // 10 GB
maxRequestSize = 10737418240L   // 100 gb
)
public class QA_register extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			String qur="insert into qa_register values(?,?,?,?,?,?,?,?,?)";
			...
			if(i>0) {
				out.print("<html><body><script>alert('Quality Analyst Registerd Successfully')</script></body></html>");
				...
				RequestDispatcher rd = request.getRequestDispatcher("QA_register.html");
				rd.include(request, response);
			}
		}catch(Exception e) {
